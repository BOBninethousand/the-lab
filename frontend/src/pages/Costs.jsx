...
export function Costs() {
  ...
  const budgetUsedPct = today && today.budget_usd > 0
    ? ((today.spend_usd / today.budget_usd) * 100).toFixed(0)
    : 0
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-4 gap-3">
        <div className="card">
          <div className="flex items-center gap-2 mb-2">
            <DollarSign size={14} className="text-lab-text-muted" />
            <span className="text-xs text-lab-text-muted">Today</span>
          </div>
          <div className="text-stat text-xl">
            ${today?.spend_usd?.toFixed(2) || '0.00'}
          </div>
          <div className="mt-2">
            <MiniBar
              value={today?.spend_usd || 0}
              max={today?.budget_usd || 5}
              color={today?.over_budget ? 'bg-lab-error' : 'bg-lab-success'}
            />
          </div>
          <div className="text-xs text-lab-text-faint mt-1">
            {budgetUsedPct}% of ${today?.budget_usd?.toFixed(2) || '5.00'} budget
          </div>
        </div>
        ...
        <div className="card">
          ...
          <div className="text-xs text-lab-text-faint mt-2">
            {summary?.total_calls || 0} API calls
          </div>
        </div>
        ...
        <div className="card">
          <div className="flex items-center gap-2 mb-2">
            {today?.over_budget ? (
              <AlertTriangle size={14} className="text-lab-error" />
            ) : (
              <DollarSign size={14} className="text-lab-text-muted" />
            )}
            <span className="text-xs text-lab-text-muted">Budget Left</span>
          </div>
          <div className={`text-stat text-xl ${today?.over_budget ? 'text-lab-error' : ''}`}>
            ${summary?.budget_remaining_usd?.toFixed(2) || '0.00'}
          </div>
          <div className="text-xs text-lab-text-faint mt-2">
            Daily limit: ${today?.budget_usd?.toFixed(2) || '5.00'}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        {/* Cost by Agent */}
        <div>
          <h2 className="text-section-label mb-4">Cost by Agent</h2>
              ...
              summary.by_agent.map(agent => (
                <div key={agent.name} className="border border-lab-border rounded-lg p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-sm font-medium text-lab-text-primary">${agent.cost.toFixed(4)}</span>
                  </div>
                  <MiniBar
                    value={agent.cost}
                    max={summary.by_agent[0]?.cost || 1}
                  />
                  <div className="flex items-center justify-between mt-2">
                    <span className="text-xs text-lab-text-muted">{agent.calls} calls</span>
        ...
        {/* Cost by Provider */}
        <div>
          <h2 className="text-section-label mb-4">Cost by Provider</h2>
          <div className="space-y-3">
              ...
              summary.by_provider.map(prov => (
                <div key={prov.provider} className="border border-lab-border rounded-lg p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-sm font-medium text-lab-text-primary">${prov.cost.toFixed(4)}</span>
                  </div>
                  <MiniBar
                    value={prov.cost}
                    max={summary.by_provider[0]?.cost || 1}
                    color={prov.provider === 'ollama' ? 'bg-lab-success' : 'bg-lab-accent'}
                  />
                  <div className="flex items-center justify-between mt-2">
                    <span className="text-xs text-lab-text-muted">{prov.calls} calls</span>
      ...
      <div>
        ...
        <div className="border border-lab-border rounded-lg overflow-hidden">
          {/* Header */}
          <div className="grid grid-cols-12 gap-4 px-4 py-3 border-b border-lab-border text-xs font-semibold uppercase tracking-wider text-lab-text-muted">
          ...
          {(scorecard?.agents || []).length === 0 ? (
            ...
            scorecard.agents.map(agent => (
              <div
                key={agent.name}
                className="grid grid-cols-12 gap-4 px-4 py-3 border-b border-white/[0.04] hover:bg-white/[0.02] transition-subtle last:border-0"
              >
          ...
          {scorecard && (
            <div className="grid grid-cols-12 gap-4 px-4 py-3 bg-lab-surface text-xs font-medium">
              ...
              <div className="col-span-2 text-lab-text-secondary text-right">${scorecard.total_cost_usd.toFixed(4)}</div>
              <div className="col-span-3" />
            </div>
          )}
        </div>
      ...
      {/* Daily Spend Chart (simplified) */}
      {summary?.daily_spend?.length > 0 && (
        <div>
          <h2 className="text-section-label mb-4">Daily Spend</h2>
          <div className="border border-lab-border rounded-lg p-4">
            <div className="flex items-end gap-1 h-32">
              {summary.daily_spend.map(day => {
                const maxSpend = Math.max(...summary.daily_spend.map(d => d.cost_usd), 0.01)
                const height = Math.max(4, (day.cost_usd / maxSpend) * 100)
                return (
